...
import {
  BookOpen, Plus, Users, Trash2, StopCircle,
...
export default function InternshipDrivesManager() {
  const router = useRouter();
  const [search, setSearch] = useState('');
  const [status, setStatus] = useState('');

  const { data, isLoading, mutate } = useSWR('/admin/internship-drives', fetcher);
  ...
  return (
    <AdminDashboardLayout>
      <Head><title>Internship Drives | Admin</title></Head>

      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-black text-[#0d1635] flex items-center gap-2">
            <BookOpen size={28} className="text-[#1B2A6B]"/> Internship Drives Approvals
          </h1>
          <p className="text-gray-500 text-sm mt-1 font-semibold">
            Review and approve campus internship drives created by colleges.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Drive Details', 'College', 'Duration', 'Status', 'Actions'].map(h => (
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                <tr key={drive.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-bold text-gray-800 text-sm">{drive.duration || 'N/A'}</span>
                  </td>
                  <td className="px-6 py-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={async () => {
                        if (confirm('Delete this drive?')) {
                          try {
                            await api.delete(`/admin/internship-drives/${drive.id}`);
                            mutate();

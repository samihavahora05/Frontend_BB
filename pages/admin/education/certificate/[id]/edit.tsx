...
export default function EditTemplatePage() {
  ...
  const { data: templates, mutate: mutateTemplates } = CertificateApiService.useTemplates();
  ...
  const [showTitle, setShowTitle] = useState('yes');
  ...
  // Load template details when id is available
  useEffect(() => {
    if (!id) return;
    let found: any = null;
    ...
    if (stored) {
      ...
      found = list.find((item: any) => String(item.id) === String(id) || item.title === id);
    }

    // Fallback to API templates
    if (!found && templates && Array.isArray(templates)) {
      found = templates.find((t: any) => String(t.id) === String(id));
    }

    if (found) {
      const bg = found.bg_image || found.background_image;
      setPreviewUrl(bg || null);
      if (bg) {
        getImageDimensions(bg).then(setImageDimensions).catch(console.error);
      }

      const layout = found.layout_settings || found;
      setTitle(found.title || layout.title || '');
      setShowTitle(found.show_title || layout.showTitle || 'yes');

      // Normalize elements for multi-element editing
      const normalizedElements = normalizeCertificateElements(found);
  ...
  const handleDeleteElement = (elId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (elements.length <= 1) {
      return toast.error('Template must have at least one text element');
    }
    const filtered = elements.filter((el) => el.id !== elId);
  ...
  const handleToggleElement = (elId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setElements((prev) =>
      prev.map((el) => (el.id === elId ? { ...el, enabled: !el.enabled } : el))
    );
  };
  ...
  const handleDownloadPreview = async () => {
    if (!previewUrl) return toast.error('No background image available');
    try {
    ...
    try {
      ...
      if (stored) {
        ...
        const updatedList = list.map((item: any) =>
          String(item.id) === String(id) || item.title === id ? { ...item, ...updatedTemplate } : item
        );
        localStorage.setItem('bb_cert_templates_v1', JSON.stringify(updatedList));
      }
      ...
      data.append('layout_settings', JSON.stringify(updatedTemplate.layout_settings));

      if (id) {
        await CertificateApiService.updateTemplate(id as string, data);
      }
  ...
  return (
    <AdminDashboardLayout>
      <Head>
        <title>Edit Certificate Template | BlueBoxx DA</title>
      </Head>
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-slate-800 tracking-tight uppercase">EDIT CERTIFICATE TEMPLATE</h1>
              <p className="text-xs text-slate-500 font-medium">Update positions, typography, dynamic fields, or replace background image.</p>
            </div>
          </div>
        </div>
          ...
          <div className="lg:w-[480px] shrink-0 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6 max-h-[85vh] overflow-y-auto custom-scrollbar">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">
                REPLACE BACKGROUND IMAGE <span className="text-[10px] text-slate-400">(OPTIONAL)</span>
              </label>
              <div className="flex border border-slate-200 rounded-xl overflow-hidden mb-2">
                <input 
                  type="text" 
                  readOnly 
                  value={backgroundFile ? backgroundFile.name : 'KEEP EXISTING BACKGROUND'} 
            ...
            >
              {isSubmitting ? 'UPDATE...' : 'UPDATE TEMPLATE'}
            </button>
            ...
            <div className="w-full min-h-[500px] bg-slate-100/70 border border-slate-200 rounded-2xl p-4 flex items-center justify-center relative overflow-hidden flex-1 select-none">
              {previewUrl ? (
                ...
                <div className="flex flex-col items-center justify-center text-slate-400 p-8 text-center">
                  ...
                  <span className="text-xs font-extrabold text-slate-500 uppercase tracking-wide">Loading Template Background</span>
                </div>
              )}
            </div>

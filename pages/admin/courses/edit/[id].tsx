...
export default function AdminEditCoursePage() {
  ...
  useEffect(() => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        if (course) {
          setForm({
            thumbnail: course.thumbnail ? getImageUrl(course.thumbnail) : "",
            title: course.title || "",
            ...
            expert_id: course.expert_id ? String(course.expert_id) : "",
  ...
  const submitCourse = async (saveAsDraft: boolean) => {
    ...
    try {
      ...
      toast.success(saveAsDraft ? "Course draft updated" : "Course updated successfully!");
      router.push("/admin/courses");
    } catch (e: any) {
      const errData = e?.response?.data;
      const msg = errData?.errors
        ? Object.values(errData.errors).flat().join(" ")
        : (errData?.message || "Failed to update course. Check fields.");
      toast.error(msg);
      setIsDrafting(false);
      ...
      <form onSubmit={handlePublish} className="max-w-6xl mx-auto pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 font-medium text-sm ml-7">Update and manage course details.</p>
          </div>

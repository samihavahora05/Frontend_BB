...
export default function CompanyPerformancePage() {
  ...
  const { data: perfResponse, isLoading, mutate } = useSWR(
    `/company/performance${queryParams.toString() ? '?' + queryParams.toString() : ''}`,
    fetcher
  );

  const { data: internDetailResponse, isLoading: loadingDetail } = useSWR(
    selectedInternId ? `/company/performance/${selectedInternId}` : null,
    fetcher
  );
  ...
  return (
    <CompanyDashboardLayout>
      <Head>
        <title>Intern Performance Tracking | Blueboxx</title>
      </Head>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm font-medium text-slate-500 mt-1">
              Real-time performance analytics calculated from your approved interns' evaluated tasks and submissions.
            </p>
          </div>
          <button
            onClick={() => mutate()}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
            ...
            <div className="mt-3">
              ...
              <p className="text-xs text-slate-500 mt-1 font-medium">Officially assigned to company</p>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
            ...
            <div className="mt-3">
              ...
              <p className="text-xs text-slate-500 mt-1 font-medium">Across all completed & graded tasks</p>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
            ...
            <div className="mt-3">
              ...
              <p className="text-xs text-slate-500 mt-1 font-medium">
                {summary?.tasks_pending_review ?? 0} task(s) pending review
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
            ...
            <div className="mt-3">
              {summary?.highest_performer ? (
                <div>
                  <p className="text-lg font-black text-slate-900 truncate">{summary.highest_performer.name}</p>
                  <span className="inline-block mt-1 text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">
                    {summary.highest_performer.average_marks}% Score
                  </span>
                </div>
              ) : (
                <div>
                  <span className="text-sm font-semibold text-slate-400">No evaluations yet</span>
                  <p className="text-xs text-slate-400 mt-1">Grade tasks to see top rank</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row gap-3 items-center justify-between">
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              placeholder="Search by intern name, email, or role..."
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <label className="text-xs font-bold text-slate-500 shrink-0">Performance:</label>
            <select
              value={rangeFilter}
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50/80 border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-6 py-3.5">Internship Program</th>
                  <th className="px-6 py-3.5 text-center">Tasks Assigned</th>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="py-16 text-center">
                      ...
                      <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                        Performance data will appear here once applicants are approved and task deliverables are reviewed.
                      </p>
                    </td>
                  </tr>
                ) : (
                  interns.map((intern: any) => (
                    <tr key={intern.application_id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <p className="font-semibold text-slate-800">{intern.internship_title}</p>
                        <p className="text-slate-400 text-[11px]">
                          Approved: {intern.approved_at ? new Date(intern.approved_at).toLocaleDateString() : 'Active'}
                        </p>
                      </td>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-500">
                    {selectedDetail?.internship?.title || 'Internship Program'}
                  </p>
                </div>
              </div>
              <button
                              ...
                              <tr key={task.id} className="hover:bg-slate-50/80">
                                ...
                                <td className="px-4 py-3">
                                  <p className="text-slate-600 italic text-xs">
                                    {task.feedback || (task.status === 'completed' ? 'No remarks provided' : 'Pending evaluation')}
                                  </p>
                                </td>
                              </tr>
                            ))}

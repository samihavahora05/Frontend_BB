...
export default function CreatePlacementDrive() {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [blueboxxAdminId, setBlueboxxAdminId] = useState<number | null>(null);

  // Fetch the BlueBoxx admin user ID on mount
  useEffect(() => {
    api.get('/college/blueboxx-admin').then(res => {
      if (res.data?.data?.id) {
        setBlueboxxAdminId(res.data.data.id);
      }
    }).catch(() => {
      // fallback: just set a dummy so form still works
    });
  }, []);
  ...
  return (
    <CollegeDashboardLayout>
      <div className="flex items-center gap-4 mb-8">
        <Link href="/college/placement-drives" className="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
          <ArrowLeft size={16} className="text-slate-500" />
        </Link>
        <div>
          <h1 className="text-2xl font-black text-slate-800 mb-1">Create Placement Drive</h1>
          <p className="text-slate-500 font-medium text-sm">Set up a new campus placement drive powered by BlueBoxx DA.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 md:col-span-2">
              <label className="text-sm font-bold text-slate-700">Drive Title</label>
              <input
                required
                type="text"
                placeholder="e.g. BlueBoxx Campus Hiring Drive 2026"
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <p className="text-[11px] text-slate-400">All placement drives are created in collaboration with BlueBoxx DA only.</p>
            </div>
          ...
          <div className="pt-6 border-t border-slate-100 flex justify-end gap-3">
            <Link href="/college/placement-drives" className="h-11 px-6 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm flex items-center hover:bg-slate-50 transition-colors">
              Cancel
            </Link>
            <button
              disabled={loading}

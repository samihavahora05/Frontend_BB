...
export default function ResumeBuilderPage() {
  ...
  const populateData = (rd: any) => {
    ...
    if (Array.isArray(rd.eduList) && rd.eduList.length > 0) setEduList(rd.eduList);
    if (Array.isArray(rd.expList) && rd.expList.length > 0) setExpList(rd.expList);
    if (Array.isArray(rd.internshipList) && rd.internshipList.length > 0) setInternshipList(rd.internshipList);
    if (Array.isArray(rd.projectList) && rd.projectList.length > 0) setProjectList(rd.projectList);
    if (rd.skillCategories) setSkillCategories(rd.skillCategories);
    if (Array.isArray(rd.certList) && rd.certList.length > 0) setCertList(rd.certList);
    if (rd.achievements) setAchievements(rd.achievements);
    if (rd.languages) setLanguages(rd.languages);
    if (Array.isArray(rd.customSections)) setCustomSections(rd.customSections);
  ...
  useEffect(() => {
    try {
      const savedLocal = localStorage.getItem("jobseeker_resume_data");
      if (savedLocal) {
        populateData(JSON.parse(savedLocal));
      }
    } catch (e) {}
    ...
    api.get("/jobseeker/profile")
      .then((res) => {
        ...
        const rawJson = p?.about_me || p?.bio;
        if (rawJson && (rawJson.startsWith('{') || rawJson.startsWith('['))) {
          try {
            populateData(JSON.parse(rawJson));
          } catch (e) {
      ...
      .catch(() => {
        api.get("/profile")
          .then((res) => {
            const raw = res.data?.data?.bio;
            if (raw && raw.startsWith('{')) {
              try { populateData(JSON.parse(raw)); } catch(e) {}
            }
          })
          .catch(() => {});
      });
  ...
  const handleSaveResume = async () => {
    try {
      ...
      localStorage.setItem("jobseeker_resume_data", JSON.stringify(resumeData));

      try {
        await api.put("/jobseeker/profile", {
          about_me: JSON.stringify(resumeData),
          phone,
          ...
          portfolio,
          headline: jobTitle,
          skills: Object.values(skillCategories).flat()
        });
      } catch (err) {
    ...
    <JobseekerDashboardLayout>
      <style jsx global>{`
        @media print {
          ...
          header, nav, aside, footer, .no-print, button, .JobseekerDashboardLayout_sidebar {
